...
const char *cAuthor = "Justin Lucker <jlucker@asu.edu>";
const char *cBinary = "bimpie";
const char *cVersion = "1.0 (2013.31.10)";
...
static void Run(tCmdLine *pCmdLine)
{
	...
	// file name.
	result = BmpWrite(pCmdLine->o ? pCmdLine->outFile : pCmdLine->inFile, &bmp);
	if (result == ErrorFileWrite)
		ErrorExit(result, "writing to %s failed", pCmdLine->outFile);
